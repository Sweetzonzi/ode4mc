...
import cn.solarmoon.spark_core.api.phys.ode.DHingeJoint;
...
//		joint.anchor2.v[3] = 0;
	...
	public void dJointSetHingeAxisOffset( double x, double y, double z, double dangle )
	{
		...
		DRotation.dQFromAxisAndAngle(qAngle, x, y, z, dangle);
		DRotation.dQMultiply3(qOffset, qAngle, qrel);
...
//		axis.v[2] *= torque;
	...
	computeInitialRelativeRotation()
	{
		if ( node[0].body != null)
		{
			if ( node[1].body  != null)
			{
				DRotation.dQMultiply1( qrel, node[0].body._q, node[1].body._q );
			}
			else
			{
